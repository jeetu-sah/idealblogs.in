      ...
      <div class="col-md-8">
        <div class="card my-4">
          @if(!empty($post_detail->image))
            <img class="card-img-top" src="{{ $post_detail->image_url }}" alt="Card image cap">
          @endif  
          <div class="card-body">
            <h2 class="card-title">{{ $post_detail->title }}</h2>
			 ...
			 @else
             @endif
            <!--<p class="card-text <?php if(empty($post_detail->image)) echo "text-center"; ?>">{{ $post_detail->description }}</p>-->
          </div>
